...
public class RateLimitFilter implements GatewayFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        ApiKeyAuthFilter.ValidationResult auth = exchange.getAttribute(ApiKeyAuthFilter.AUTH_RESULT_KEY);
        String identifier = (auth != null && auth.keyId() != null)
                ? auth.keyId()
                : Objects.requireNonNullElse(exchange.getRequest().getRemoteAddress(), "unknown").toString();

        int limit = (auth != null && auth.rateLimit() > 0) ? auth.rateLimit() : proxy.getRateLimitRequests();
