...
public class GatewayConfig {
   ...
   * Gateway re-subscribes when RefreshRoutesEvent fires.
   */
  @Bean
                                  ...
                                  ex ->
                                      Mono.just(
                                          ex.getRequest()
                                              .getURI()
                                              .getPath()
                                              .startsWith(proxy.getPathPrefix())))
                              .filter(
                                  new OrderedGatewayFilter(
                                      (ex, chain) -> {
                                        String path = ex.getRequest().getURI().getPath();
                                        String stripped = path;
                                        if (proxy.isStripPrefix()
                                            && path.startsWith(proxy.getPathPrefix())) {
                                        ...
                                        return apiKeyAuthFilter.filter(
                                            mutated, next -> rateLimitFilter.filter(next, chain));
                                      },
                                      1))
                              .build();
                        }));
