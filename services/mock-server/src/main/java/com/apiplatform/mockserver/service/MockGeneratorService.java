...
public class MockGeneratorService {
  ...
  public MockResponse generate(
      Map<String, Object> oasDoc, String method, String path, int statusCode) {
    // Parse OAS from map
    OpenAPI openAPI = parseOas(oasDoc);
    if (openAPI == null) {
      return new MockResponse(
          404, Map.of("error", "Cannot parse OAS document"), "application/json");
    }
  ...
  private List<Object> generateArray(Schema<?> schema, int depth) {
    ...
    return depth == 0 ? List.of(item, generateFromSchema(items, depth + 1)) : List.of(item);
  }
  ...
  private PathItem findPathItem(OpenAPI openAPI, String requestPath) {
    ...
    for (Map.Entry<String, PathItem> entry : openAPI.getPaths().entrySet()) {
      String pattern = entry.getKey().replaceAll("\\{[^/]+}", "[^/]+").replace("/", "\\/");
